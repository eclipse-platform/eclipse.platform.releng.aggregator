...
if [[ -z "${currentStream}" ]]
then
  printf "\n\t%s\t%s\n" "WARNING:" "Current stream version not specified on command line, assuming 4.16"
  currentStream="4.16"
fi
...
then
  printf "\n\t%s\t%s\n" "WARNING:" "Maintenance stream version not specified on command line, assuming 4.15"
  maintenanceStream="4.15"
fi
